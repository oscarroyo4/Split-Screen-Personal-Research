...
Go to the repository of the [Slpit Screen](https://github.com/oscarroyo4/Split_Screen-Personal_Research).
...
It's an option to implement when we have multiple primary focus points that we need to have them at the same time on the screen and under control. Some exemples of software can be any 3D modeling program. In video games, the common case is a game with local multiplayer, but not all local multiplayer games needs a split screen. If the players can not separate from each other and leave the screen on the stage, you can search for an alternative in split screen.

Some exemples of that, are games like *overcoocked* or *Cuphead*, that the players can not go out off the screen. Examples of local multiplayer games that need a split screen are any racing game or shooter, both players need the control of their character and camera.
...
This is the tipically option that the games do when they don't want to imlement a split screen. Games like *Wizard of Legends* or "Unravel" want the players to stay together, blocking the option to separate.
...
This is a radical option. Only usefull in games that we want to punish the player for going off screen. Competitive figthing games like *Smash Bros* implement this option because the objective is to avoid leaving the map and throwing the opponent out. In general cases, it's very anoying to the player.
...
If the player is pushing an edge of the screen, he drags the screen and the other players. This option continues to affect the player's gameplay, not only the player's own gameplay, but also the other players. 
