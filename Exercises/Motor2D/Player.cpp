...
Player::Player(fPoint pos) : Entity(pos)
{
...
bool Player::Start()
{
	...
	case 3:
		...
		break;
	default:
		LOG("Number of players is greater than 3. You probably restarted the game and need to set the variable to 0 again.");
		break;
	}
...
void Player::Movement(float dt)
{
	...
	float magnitude = sqrtf((iso_dir.x * iso_dir.x) + (iso_dir.y * iso_dir.y));
	fPoint iso_dirN(iso_dir.x / magnitude, iso_dir.y / magnitude);

	velocity = iso_dirN * speed * dt;
...
void Player::InputMovementKeyboard(fPoint & input)
{
